...
#_*****************************************************************************
#_*                                                                           *
#_* File name: showAllBifFig2.cgi                                             *
#_* cgi for creating methylation pattern figure 2                             *
#_* (black/white circle figure with center line)                              *
...
if ($data->{showcpg2}) {
    ...
    if ($data->{revpos2}) {
         $min = $len - $cpg[@cpg-1] - 1 + $data->{spos2};
         $max = $len - $cpg[0]      - 1 + $data->{spos2};
    } else {
         $min = $cpg[0]      + $data->{spos2};
         $max = $cpg[@cpg-1] + $data->{spos2};
    }
...
$y += int (3*$data->{eachh}/4)
    if ($data->{showno2} && ! $data->{cpgno2});

$width  = $data->{eachw} * $cpgn + FIGMRG * 2;
$heigh = $data->{eachh} * @ord;
$heigh += int ($posdi + $data->{diame} * FRATIO/2 * $mlen + 1),
    if ($data->{showcpg2});
$heigh += int (3*$data->{eachh}/4) if ($data->{showno2});
...
foreach $ord (@ord) {
    ...
    $x = FIGMRG;
    $cy = $y + int(($data->{eachh} + 1 - $data->{center} + 1)/2);
...
$x = int($data->{eachw}/2) + FIGMRG;

for ($i = 0; $data->{showno2} && $i < $cpgn; $i++, $x+= $data->{eachw}) {

    $j = $data->{revno2} ? $cpgn - $i : $i + 1;
...
for ($i = 0, $x = FIGMRG; $data->{showcpg2} && $i < $cpgn; $i++, $x+= $data->{eachw}) {

    $pos = $cpg[$i];
    $pos = $len - $pos - 1 if ($data->{revpos2});
    $pos += $data->{spos2};
