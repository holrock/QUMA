...
#_*****************************************************************************
#_*                                                                           *
#_* File name: makeCompFig5.cgi                                               *
#_* cgi for creating methylation status comparison graph 5                    *
...
$gd->line ($x1, int((HIGHT*3 - GMARGB2*3 + GMARGH)/4),
	   $x2, int((HIGHT*3 - GMARGB2*3 + GMARGH)/4),
	   $black);

$gd->line ($x+1, HIGHT - GMARGB2,
	   $x2,  HIGHT - GMARGB2,
	   $black);

$gd->line ($x2, HIGHT - GMARGB2,
	   $x2, int((HIGHT*3 - GMARGB2*3 + GMARGH)/4),
	   $black);
...
for ($i = 0; $i < @pos; $i++) {
    ...
    if ($meth{$pos[$i]}->{ratio1} > 0.5 && $meth{$pos[$i]}->{ratio2} > 0.5) {
	if ($meth{$pos[$i]}->{ratio1} > $meth{$pos[$i]}->{ratio2}) {
	    $gd->line ($x, int ((HIGHT - GMARGB2 + GMARGH) / 2),
		       $x, int (HIGHT - GMARGB2 - (HIGHT - GMARGB2 - GMARGH) * $meth{$pos[$i]}->{ratio1}),
		       $blue);
	} else {
	    $gd->line ($x, int ((HIGHT - GMARGB2 + GMARGH) / 2),
		       $x, int (HIGHT - GMARGB2 - (HIGHT - GMARGB2 - GMARGH) * $meth{$pos[$i]}->{ratio2}),
		       $blue);
	}

    } elsif ($meth{$pos[$i]}->{ratio1} < 0.5 && $meth{$pos[$i]}->{ratio2} < 0.5) {
	if ($meth{$pos[$i]}->{ratio1} < $meth{$pos[$i]}->{ratio2}) {
	    $gd->line ($x, int ((HIGHT - GMARGB2 + GMARGH) / 2),
		       $x, int (HIGHT - GMARGB2 - (HIGHT - GMARGB2 - GMARGH) * $meth{$pos[$i]}->{ratio1}),
		       $blue);
	...
	$gd->line ($x, int (HIGHT - GMARGB2 - (HIGHT - GMARGB2 - GMARGH) * $meth{$pos[$i]}->{ratio1}),
		   $x, int (HIGHT - GMARGB2 - (HIGHT - GMARGB2 - GMARGH) * $meth{$pos[$i]}->{ratio2}),
		   $blue);
    }

    if ($meth{$pos[$i]}->{ratio1} > 0.5) {
	for ($j = $x1; $j <= $x-1; $j++) {
	    $gd->line ($j, int ((HIGHT - GMARGB2 + GMARGH) / 2),
		       $j, int (HIGHT - GMARGB2 - (HIGHT - GMARGB2 - GMARGH) * $meth{$pos[$i]}->{ratio1}),
		       $black);

	}
    } else {
	$gd->line ($x1,  int (HIGHT - GMARGB2 - (HIGHT - GMARGB2 - GMARGH) * $meth{$pos[$i]}->{ratio1}),
		   $x-1, int (HIGHT - GMARGB2 - (HIGHT - GMARGB2 - GMARGH) * $meth{$pos[$i]}->{ratio1}),
		   $black);
	$gd->line ($x1, int ((HIGHT - GMARGB2 + GMARGH) / 2),
		   $x1, int (HIGHT - GMARGB2 - (HIGHT - GMARGB2 - GMARGH) * $meth{$pos[$i]}->{ratio1}),
		   $black);
    }

    if ($meth{$pos[$i]}->{ratio2} > 0.5) {
	for ($j = $x+1; $j <= $x2; $j++) {
	    $gd->line ($j, int ((HIGHT - GMARGB2 + GMARGH) / 2),
		       $j, int (HIGHT - GMARGB2 - (HIGHT - GMARGB2 - GMARGH) * $meth{$pos[$i]}->{ratio2}),
		       $black);
	}
    } else {
	$gd->line ($x2,  int (HIGHT - GMARGB2 - (HIGHT - GMARGB2 - GMARGH) * $meth{$pos[$i]}->{ratio2}),
		   $x+1, int (HIGHT - GMARGB2 - (HIGHT - GMARGB2 - GMARGH) * $meth{$pos[$i]}->{ratio2}),
		   $black);
	$gd->line ($x2, int ((HIGHT - GMARGB2 + GMARGH) / 2),
		   $x2, int (HIGHT - GMARGB2 - (HIGHT - GMARGB2 - GMARGH) * $meth{$pos[$i]}->{ratio2}),
		   $black);
    }
}
...
if ($data->{down}) {
    ...
    $name .= "_compared_CpG_status_graph5.";
